...
function createReminder( $nome , $descricao , $data_vencimento){ //arrunar
    $conn = create_connection();
    $usuario_id = $_SESSION['id'];
    $descricao = pg_escape_string($conn , $descricao);
    $query = "INSERT INTO lembretes (usuario_id , nome , descricao , data_vencimento)
              VALUES ($1 ,$2 , $3 , $4)";
    $stmt_name = 'create_reminder_' . uniqid();
    pg_prepare($conn , $stmt_name , $query);
    $result = pg_execute($conn , $stmt_name , [$usuario_id , $nome , $descricao , $data_vencimento]);
